...
class File():
    ...
    def get_files_indir(self):
        ...
        try:
            for file in files:
                if (".txt" in file) and (self.in_prefix in file):
    ...
    def checkline(self, line):
        ...
        if (("公開" in line) and ("年" in line) and ("月" in line)):
            flag += 1
        if (("修正" in line) and ("年" in line) and ("月" in line)):
            flag += 1
        if (("発行" in line) and ("年" in line) and ("月" in line)):
            flag += 1
...
def test():
    ...
    myfile = File(in_prefix, out_prefix)

    fname = "mecab_utf8_oshieto_tabisuru_otoko.txt"
